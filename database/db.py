...
async def get_user_by_id(session, discord_id):
    async with session as session:
        user = await session.execute(select(User).where(User.discord_id == discord_id))
        return user.scalar_one_or_none()


async def get_event_by_id(session, message_id):
    async with session as session:
...
async def get_events_by_author_id(session, discord_id, page=1, per_page=10):
    async with session as session:
...
async def get_events_by_participant_id(session, discord_id, page=1, per_page=10):
    async with session as session:
        events = await session.execute(
            select(Event)
            .options(joinedload(Event.participants))
...
async def get_participant(session, discord_id, event_id):
    async with session as session:
...
async def remove_event(session, message_id):
    async with session as session:
        event = await get_event_by_id(session, message_id)
        if event:
            await session.delete(event)
            await session.commit()


async def remove_participant(session, participant_id, event_id):
    async with session as session:
        participant = await get_participant(session, participant_id, event_id)
        if participant:
